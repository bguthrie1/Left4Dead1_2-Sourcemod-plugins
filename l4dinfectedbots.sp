		...
		if (GameMode != VERSUS)
		{
			for (new i=1; i<=MaxClients; i++)
			{
				...
				if (GetClientTeam(i)==TEAM_INFECTED)
				{
					...
					if (!IsFakeClient(i))
					{
						if (GameMode != VERSUS && GetConVarBool(h_JoinableTeams) && !GetConVarBool(h_AdminJoinInfected))
						{
...
public Action:evtPlayerFirstSpawned(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (!b_LeftSaveRoom)
	{
		if (GetConVarBool(h_JoinableTeams) && GameMode != VERSUS || GetConVarBool(h_VersusCoop) && GameMode == VERSUS)
		{
			if (g_bIsL4D2)
			{
				SetConVarInt(FindConVar("sb_all_bot_game"), 1);
...
public Action:evtRoundEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (!b_HasRoundEnded)
	{
		...
		if (!g_bIsL4D2 && GameMode == VERSUS && !RealPlayersOnSurvivors() && !AllSurvivorsDeadOrIncapacitated())
		{
...
{
	if (GameMode == SURVIVAL || SurvivalVersus)
	{  
...
{
	if (g_bIsL4D2 || b_LeftSaveRoom || GameMode != VERSUS || RealPlayersOnInfected() || TempBotSpawned)
		return Plugin_Continue;
...
public Action:evtPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (DirectorSpawn && GameMode != VERSUS)
	{
		...
		else if (IsPlayerSpitter(client) && g_bIsL4D2)
		{
			if (IsFakeClient(client))
			{
				if (!SpecialHalt)
		...
		else if (IsPlayerJockey(client) && g_bIsL4D2)
		{
			if (IsFakeClient(client))
			{
				if (!SpecialHalt)
		...
		else if (IsPlayerCharger(client) && g_bIsL4D2)
		{
			if (IsFakeClient(client))
			{
				if (!SpecialHalt)
	...
	{
		if (g_bIsL4D2 && GameMode == COOP && IsFakeClient(client) && RealPlayersOnInfected() && !DisallowTankFunction)
		{
		...
		{	
			...
			if (!TankFrustStop)
			{
				...
				if (GameMode == SURVIVAL)
				{
					...
					{
						if (g_bIsL4D2 && !AreTherePlayersWhoAreNotTanks() && GetConVarBool(h_CoopPlayableTank) && !DisallowTankFunction || g_bIsL4D2 && !GetConVarBool(h_CoopPlayableTank) && !DisallowTankFunction)
						{
							CreateTimer(0.1, TankBugFix, client);
							CreateTimer(0.2, kickbot, client);
						}
	...
	if (g_bIsL4D2 && !AlreadyGhosted[client])
	{
		AlreadyGhosted[client] = true;
		InfectedForceGhost(client);
	}
	else if (!g_bIsL4D2 && GameMode != VERSUS && !IsFakeClient(client) && !IsPlayerTank(client) && (GetConVarBool(h_FreeSpawn) && !AlreadyGhosted[client] || FreeSpawnReset[client]))
	{
		AlreadyGhosted[client] = true;
	...
	if (IsFakeClient(client) && GameMode == VERSUS && !IsPlayerTank(client))
	...
	// on versus.
	if (!g_bIsL4D2 && GameMode != VERSUS && !IsFakeClient(client))
	{
	...
	else if (g_bIsL4D2 && GameMode != VERSUS && !IsFakeClient(client))
	{
...
public Action:evtPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	*/
	...
	if (GetEventBool(event, "victimisbot") && (GameMode == VERSUS) && (!DirectorSpawn))
	{
		if (!IsPlayerTank(client))
		{
	...
	if (!g_bIsL4D2 && !IsFakeClient(client) && !GetConVarBool(h_StatsBoard) && GameMode != VERSUS)
	{
...
public Action:evtPlayerTeam(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (!b_HasRoundEnded && b_LeftSaveRoom && GameMode == VERSUS)
	{
	...
	else if ((newteam == 3 || newteam == 1) && GameMode != VERSUS)
	{
		...
		if (GetClientTeam(client) == TEAM_INFECTED)
		{
			if (IsPlayerTank(client) && isTankOnFire[client] == false)
			{
				...
				if ((GetEntProp(client, Prop_Data, "m_fFlags") & FL_ONFIRE) && PlayerIsAlive(client))
				{
					...
					if ((StrContains(difficulty, "Easy", false) != -1) && (GameMode == COOP))
					{
						if (g_bIsL4D2)
					...
					else if ((StrContains(difficulty, "Normal", false) != -1) && (GameMode == COOP))
					{
						if (g_bIsL4D2)
							fireTankExpiry = FindConVar("tank_burn_duration");
						else
					...
					else if ((StrContains(difficulty, "Hard", false) != -1) && (GameMode == COOP))
					{
						fireTankExpiry = FindConVar("tank_burn_duration_hard");
					}
					else if ((StrContains(difficulty, "Impossible", false) != -1) && (GameMode == COOP))
					{
						fireTankExpiry = FindConVar("tank_burn_duration_expert");
					}
					else if (GameMode == VERSUS || GameMode == SURVIVAL)
					{
						if (g_bIsL4D2)
							fireTankExpiry = FindConVar("tank_burn_duration");
						else
...
public Action:evtInfectedWaitSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		decl timetowait;
		if (GameMode == VERSUS && !IsFakeClient(client))
		{	
			timetowait = GetEventInt(event, "spawntime");
		}
		else if (GameMode != VERSUS && !IsFakeClient(client))
		{	
...
{
	if (client && (GameMode == COOP || GameMode == SURVIVAL) && GetConVarBool(h_JoinableTeams))
	{
...
{
	if (client && (GameMode == COOP || GameMode == SURVIVAL))
	{
		SwitchToSurvivors(client);
	}
}
...
public Action:AnnounceJoinInfected(Handle:timer, any:client)
{
	...
	{
		if ((GetConVarBool(h_JoinableTeamsAnnounce)) && (GetConVarBool(h_JoinableTeams)) && ((GameMode == COOP) || (GameMode == SURVIVAL)))
		{
	...
	if (IsClientInGame(coward) && IsFakeClient(coward) && GetClientTeam(coward) == TEAM_INFECTED && !IsPlayerTank(coward) && PlayerIsAlive(coward))
	{
		...
		else
		{
			...
			if (!DirectorSpawn)
			{
				...
				if (GameMode == VERSUS && GetConVarBool(h_AdjustSpawnTimes) && MaxPlayerZombies != HumansOnInfected())
					SpawnTime = SpawnTime / (MaxPlayerZombies - HumansOnInfected());
				else if (GameMode == COOP && GetConVarBool(h_AdjustSpawnTimes))
					SpawnTime = SpawnTime - TrueNumberOfSurvivors();
	...
	for (new i=1;i<=MaxClients;i++)
	{
		if (IsClientInGame(i) && (!IsFakeClient(i))) // player is connected and is not fake and it's in game ...
		{
			...
			if (GetClientTeam(i)==TEAM_INFECTED)
			{
				...
				else if (!PlayerIsAlive(i) && respawnDelay[i] > 0 && GameMode != VERSUS)
				{
					resetLife[i] = true;
...
public Action:TankSpawner(Handle:timer, any:client)
{
	...
	if (GetConVarBool(h_CoopPlayableTank) && IndexCount != 0 )
	{
		...
		if (g_bIsL4D2 && IsPlayerJockey(Index[tank]))
		{
...
{
	if (GameMode != COOP || !GetConVarBool(h_CoopPlayableTank))
		return Plugin_Continue;
		...
		if (IsClientInGame(i))
		{
			...
			if (GetClientTeam(i) == TEAM_INFECTED && PlayerIsAlive(i))
			{
				...
				if (IsPlayerSmoker(i))
					...
					tanks++;	
				else if (g_bIsL4D2 && IsPlayerSpitter(i))
					spitters++;	
				else if (g_bIsL4D2 && IsPlayerJockey(i))
					jockeys++;	
				else if (g_bIsL4D2 && IsPlayerCharger(i))
					chargers++;	
			}
		}
	}
	...
	for (new i=1;i<=MaxClients;i++)
	{
		if (IsClientInGame(i) && !IsFakeClient(i)) // player is connected and is not fake and it's in game ...
		{
			...
			if (GetClientTeam(i) == TEAM_INFECTED)
			{
				...
				else if (!PlayerIsAlive(i) && GameMode == VERSUS) // if player is just dead
				{
					resetLife[i] = true;
					SetLifeState(i, false);
				}
...
public cvarZombieHPChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	else if (g_bIsL4D2 && StrEqual(cvarStr, "z_spitter_health", false))
	{
		zombieHP[3] = StringToInt(newValue);
	}
	else if (g_bIsL4D2 && StrEqual(cvarStr, "z_jockey_health", false))
	{
		zombieHP[4] = StringToInt(newValue);
	}
	else if (g_bIsL4D2 && StrEqual(cvarStr, "z_charger_health", false))
	{
		zombieHP[5] = StringToInt(newValue);
	}
	else if (StrEqual(cvarStr, "z_tank_health", false) && GameMode == VERSUS)
	{
		zombieHP[6] = RoundToFloor(StringToInt(newValue) * 1.5);	// Tank health is multiplied by 1.5x in VS
	}
	else if (StrEqual(cvarStr, "z_tank_health", false) && GameMode != VERSUS && StrContains(difficulty, "Easy", false) != -1)
	{
		zombieHP[6] = RoundToFloor(StringToInt(newValue) * 0.75);
	}
	else if (StrEqual(cvarStr, "z_tank_health", false) && GameMode != VERSUS && StrContains(difficulty, "Normal", false) != -1)
	{
		zombieHP[6] = RoundToFloor(StringToInt(newValue) * 1.0);
	}
	else if (StrEqual(cvarStr, "z_tank_health", false) && GameMode != VERSUS && StrContains(difficulty, "Hard", false) != -1)
	{
		zombieHP[6] = RoundToFloor(StringToInt(newValue) * 2.0);
	}
	else if (StrEqual(cvarStr, "z_tank_health", false) && GameMode != VERSUS && StrContains(difficulty, "Impossible", false) != -1)
	{
		zombieHP[6] = RoundToFloor(StringToInt(newValue) * 2.0);
	}
}
...
public ShowInfectedHUD(src)
{
	...
	if (GameMode == VERSUS && !BotsAlive() && !MoreThanFivePlayers())
	{
		return;
	}
	...
	if (GameMode == VERSUS && !MoreThanFivePlayers())
	...
	if (roundInProgress)
	{
		...
		{
			if (!IsClientInGame(i)) continue;
			if (( MoreThanFivePlayers() && !IsFakeClient(i)) || IsFakeClient(i) || (GameMode != VERSUS && !IsFakeClient(i)))
			{
				if (GetClientMenu(i) == MenuSource_RawPanel || GetClientMenu(i) == MenuSource_None)
				{
					if (GetClientTeam(i) == TEAM_INFECTED)
					{
						...
						else
						{
							...
							else if (respawnDelay[i] == 0 && GameMode != VERSUS && !DirectorSpawn)
							{
							...
							else if (respawnDelay[i] > 0 && DirectorSpawn && GameMode != VERSUS)
							{
