/********************************************************************************************
* Plugin	: [L4D] Character Select Menu (Old)
* Version	: 1.3
* Game		: Left 4 Dead 
* Author	: MI 5
* Testers	: Myself
...
* 		- Survivors can now become Infected and vice versa (Character or model)
* 
* Version 1.2
...
#define PLUGIN_NAME "[L4D] Character Select Menu (Old)"
#define DEBUG 0
#pragma semicolon 1
...
public OnPluginStart()
{
	...
	// Fun Cvar
	h_Fun = CreateConVar("l4d_csm_fun_mode", "0", "If 1, players will be able to change their models on the csm", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY, true, 0.0, true, 1.0);
	
	// Activate Cvar
	g_hActivate = CreateConVar("l4d_csm_enable", "1", "If 1, character select menu is activated", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY, true, 0.0, true, 1.0);
	
	// Infected Menu cvar
	g_hInfectedMenu = CreateConVar("l4d_csm_infected_menu", "1", "If 1, Infected Menu will be shown to players", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY, true, 0.0, true, 1.0);
	
	// Survivor Menu cvar
	g_hSurvivorMenu = CreateConVar("l4d_csm_survivor_menu", "1", "If 1, Survivor Menu will be shown to players", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY, true, 0.0, true, 1.0);
	
	// Change Limit cvar
	g_hChangeLimit = CreateConVar("l4d_csm_change_limit", "9999", "Sets the amount of times a client can change their character", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY);
...
public CharMenu(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		else if (StrEqual(item, "zoey_fun"))
		{
			decl String:model[] = "models/survivors/survivor_teenangst.mdl";
			SetEntityModel(param1, model);
		...
		else if (StrEqual(item, "francis_fun"))
		{
			decl String:model[] = "models/survivors/survivor_biker.mdl";
			SetEntityModel(param1, model);
		...
		else if (StrEqual(item, "louis_fun"))
		{
			decl String:model[] = "models/survivors/survivor_manager.mdl";
			SetEntityModel(param1, model);
		...
		else if (StrEqual(item, "bill_fun"))
		{
			decl String:model[] = "models/survivors/survivor_namvet.mdl";
			SetEntityModel(param1, model);
		...
		else if (StrEqual(item, "boomer_fun") && GetClientTeam(param1) == TEAM_INFECTED)
		{
		...
		else if (StrEqual(item, "hunter_fun") && GetClientTeam(param1) == TEAM_INFECTED)
		{
		...
		else if (StrEqual(item, "tank_fun") && GetClientTeam(param1) == TEAM_INFECTED)
		{
		...
		else if (StrEqual(item, "witch_fun") && GetClientTeam(param1) == TEAM_INFECTED)
		{
			decl String:model[] = "models/infected/witch.mdl";
			SetEntityModel(param1, model);
...
public Action:PlayerMenu(Handle:timer, any:client)
{
	...
	{
		if (GetConVarBool(g_hSurvivorMenu) && g_ChangeLimitPlayer[client] >= 1)
		{
		...
		{
			AddMenuItem(menu, "zoey_fun", "Zoey(Model)");
			AddMenuItem(menu, "francis_fun", "Francis(Model)");
			AddMenuItem(menu, "louis_fun", "Louis(Model)");
			AddMenuItem(menu, "bill_fun", "Bill(Model)");
		}
		...
		if (GetClientTeam(client) == TEAM_INFECTED && GetConVarBool(h_Fun) && GetConVarBool(g_hInfectedMenu) && g_ChangeLimitPlayer[client] >= 1 || GetClientTeam(client) == TEAM_INFECTED && GetUserFlagBits(client) != 0 && GetConVarBool(g_hInfectedMenu) && g_ChangeLimitPlayer[client] >= 1)
		{
...
Spawn_Infected(InfectedTicket)
{
	...
	new CurrentGameMode = g_GameMode;
