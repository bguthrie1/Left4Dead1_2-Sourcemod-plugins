...
*      - Fixed Hunter Spawning where the hunter would spawn normally for 5 minutes into the map and then suddenly reduce the hunter spawn time
*      - Survivor Bots can now pass the areas where they got stuck in (they can move throughout the map on their own now)
*      - In versus, set vs_max_team_switches to 99 if there are no survivor players, then switch back to default if there is a player 
*      - It's easier to join the infected team in coop/survival by typing !joininfected
...
public Action:BotEnhancerTimer(Handle:Timer)
{
	if (LeavedSafeRoom)
	{
    LogMessage("[Infected bots] Checking for SurvivorBots");
    // reset counters
    SurvivorBotCount = 0;
    SurvivorRealCount = 0;
    ...
    {
      LogMessage("[Infected bots] All Survivor Bot Team, triggering entities");
    ...
    {
    LogMessage("[Infected bots] Found a Survivor Player, not executing entities and redoing check");
    CreateTimer(10.0, BotEnhancerTimer)
    }
  }
}
...
public OnClientPutInServer(client)
{
	if (client)
	{
			CreateTimer(120.0, AnnounceJoinInfected, client);
	}
}
...
CountInfected()
{
	...
	{
	AllBotsTeam = true;
	LogMessage("[Infected bots] All Infected Bot Team");
	}
	else
	{
	AllBotsTeam = false;
  ...
  if ((SurvivorRealCount == 0) && (GameMode == 2))
  {
