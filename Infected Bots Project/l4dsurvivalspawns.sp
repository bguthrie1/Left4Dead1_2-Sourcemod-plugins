...
*
* Version 1.7.2
*      - Removed autoconfig for plugin
*      - Reintroduced coop/survival playable spawns
*      - Coop/Survival spawns at conistent intervals of 20 seconds
...
*      - If there is a max of 4 SI in coop, it will only spawn 3 so that the fourth slot is saved for the tank. 4 with 5 max and so on.
*      - Timers implemented from preventing the SI from spawning right at the start
...
* 	   - Added cvar to control the spawn time of the infected bots
...
public OnPluginStart()
{
	...
	//Add a sourcemod command so players can easily join infected in coop/survival, remove the spectator before final
	...
	h_VersusHunterLimit = CreateConVar("l4d_infectedbots_versus_hunter_limit", "4", "Sets the limit for hunters spawned by the plugin, must either restart the round or change the map if changed in-game", FCVAR_PLUGIN|FCVAR_NOTIFY);
	...
	h_DirectorSpawn = CreateConVar("l4d_infectedbots_director_spawn", "0", "If 1, the director will spawn the specials instead of the plugin, only applies to coop/survival, must either restart the round or change the map if changed in-game", FCVAR_PLUGIN|FCVAR_NOTIFY);
	h_InfectedSpawnTime = CreateConVar("l4d_infectedbots_spawn_time", "20", "Sets the spawn time for infected bots spawned by the plugin", FCVAR_PLUGIN|FCVAR_NOTIFY);
	...
	//HookEvent("player_entered_checkpoint", EnterCheckPoint);
...
TweakSettings ()
{
	...
	if ((GameMode == 1) && (!DirectorSpawn))
	{
		SetConVarInt(FindConVar("z_gas_limit"), 0);
		SetConVarInt(FindConVar("z_exploding_limit"), 0);
		SetConVarInt(FindConVar("z_hunter_limit"), 0);
		SetConVarInt(FindConVar("z_minion_limit"), 32);
		ResetConVar(FindConVar("director_no_specials"), true, true);
	}
	else if ((GameMode == 1) && (DirectorSpawn))
	{
		ResetConVar(FindConVar("director_no_specials"), true, true);
		AllowBoomerBots = false
		AllowHunterBots = false
		AllowSmokerBots = false
	}
	
	//Better Versus Infected AI, MI 5 Code
	if (GameMode == 2)
	{
	...
	if ((GameMode == 3) && (!DirectorSpawn))
	{
		SetConVarInt(FindConVar("director_no_specials"), 1);
	}
	else if ((GameMode == 3) && (DirectorSpawn))
	{
		ResetConVar(FindConVar("director_no_specials"), true, true);
		AllowBoomerBots = false
		AllowHunterBots = false
...
{
	//MI 5 Code, Initiate new timer when survivors leave the safe room, which may be canceled out by the safe timer, This is to fix the survival spawn bug
	if (GameMode == 3)
	{
...
public Action:CheatReseterTwo(Handle:Timer)
{
	
	new flags5 = GetCommandFlags("z_spawn");
	SetCommandFlags("z_spawn", flags5|FCVAR_CHEAT);	
}

public Action:BotWait(Handle:Timer)
{
	// First we count the ammount of survivor real players and bots, this is to prevent the bots from leaving the safe room and triggering the bot infected when there is no player
	...
	if ((SurvivorRealCount = 0) && (InfectedRealCount = 0))
	{
		SetConVarInt(FindConVar("sb_stop"), 1);
		CreateTimer(1.0, BotWait)
	}
	else if ((SurvivorRealCount > 0) || (InfectedRealCount > 0))
	{
		//Start the Safe Room Timer for Coop/Survival, MI 5 Code, two timers for survival/coop and the timer for bots to pass cresendos
		if (GameMode == 1)
		{
...
//Allows players to easily join infected in coop/survival

public OnClientPutInServer(client)
{
	if ((client) && (GameMode != 2))
	{
...
{
	if (IsClientInGame(client) && (GameMode != 2))
	{
		PrintToChat(client, "[SM] Type !joininfected to join the infected team");
	}
}
...
BotTypeNeeded()
{
	...
	if ((boomers < MaxBoomers) && (canSpawnBoomer) && (tmpAllowBoomerBots))
		...
		return 2;
	else if ((hunters < MaxHunters) && (tmpAllowHunterBots)) // we need a hunter ???? is hunter bot allowed ??
		return 1;
...
public Action:Spawn_InfectedBot(Handle:timer)
{
	...
	if (anyclient == 0)
	{
		...
		if (anyclient == 0)
		{
			LogMessage("[L4D] Infected Bots: CreateFakeClient returned 0 -- Infected bot was not spawned");
			wait = false;
			return;
		}
		temp = true;
	...
	new flags4 = GetCommandFlags("z_spawn");
	SetCommandFlags("z_spawn", flags4 & ~FCVAR_CHEAT);
	...
	switch (bot_type)
	{
		case 0: // Nothing
		{
			LogMessage("[L4D] Infected Bots: We got nothing, trying to spawn again");
			CreateTimer(2.0, Spawn_InfectedBot, _, 0);
			InfectedBotQueue++;
		}
		case 1: // Hunter
		{
			ServerCommand("z_spawn hunter auto");
		}
		case 2: // Smoker
		{
			ServerCommand("z_spawn smoker auto");
		}
		case 3: // Boomer
		{
			ServerCommand("z_spawn boomer auto");
		}
	}
	
	// restore z_spawn
	CreateTimer(1.0, CheatReseterTwo)
