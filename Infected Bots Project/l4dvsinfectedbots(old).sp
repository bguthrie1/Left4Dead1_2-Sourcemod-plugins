...
new bool:MissionLostSet;
new bool:PlayerEntered;
new MaxInfected;
...
new Handle:h_InfectedBotsCheckerTimer;
new Handle:h_SafeTimer;
new Handle:h_BotEnhancerTimer;
new Handle:h_CheatReseter;
...
public OnPluginStart()
{
	...
	h_InfectedSpawnTime = CreateConVar("l4d_infectedbots_spawn_time", "20", "Sets spawn time for special infected spawned by the plugin", FCVAR_PLUGIN|FCVAR_NOTIFY);
...
TweakSettings ()
{
	...
	//Turns off the ability for the director to spawn infected bots, interstingly enough it does not affect tank spawns, MI
	if (GameMode == 3)
	{
...
public Action:RoundStart (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsMapVS)
	{
		...
		if (!RoundStarted)
		{
			...
			if (MissionLostSet)
			{
				LogMessage ("Timer started due to mission loss")
				//Start the Safe Room Timer for Coop/Survival, MI 5 Code, two timers for survival/coop and the timer for bots to pass cresendos, MI 5
				if (GameMode == 1)
				{
					h_SafeTimer = CreateTimer(30.0, SafeTimer);
				}
				if (GameMode == 3)
				{
					h_SafeTimer = CreateTimer(60.0, SafeTimer);
				}
				h_BotEnhancerTimer = CreateTimer(120.0, BotEnhancerTimer);
				if (GameMode != 2)
				{
				h_InfectedBotsCheckerTimer = CreateTimer(65.0, InfectedBotChecker) 
				}
			}
			TweakSettings();
		}
...
public Action:RoundEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsMapVS)
	{
		...
		if (!RoundEnded)
		{
			...
			LeavedSafeRoom = false;
			MissionLostSet = true
			CloseHandle(h_InfectedBotsCheckerTimer)
			CloseHandle(h_SafeTimer)
			CloseHandle(h_BotEnhancerTimer)
			CloseHandle(h_CheatReseter)
		}
	}
}
...
{
	// The safe room timer is in place when the round starts for coop/survival, MI 5
...
public Action:BotEnhancerTimer(Handle:Timer)
{
	if (LeavedSafeRoom)
	{
		...
		if (SurvivorRealCount == 0)
		{
			...
			h_BotEnhancerTimer = CreateTimer(5.0, BotEnhancerTimer);
			h_CheatReseter = CreateTimer(7.0, CheatReseter);
			
			
		}
		else
		{
			h_BotEnhancerTimer = CreateTimer(10.0, BotEnhancerTimer)
		}
	}
}
...
public Action:InfectedBotChecker(Handle:Timer)
{
	...
	if (GameMode != 2)
	{
		...
		h_InfectedBotsCheckerTimer = CreateTimer(20.0, InfectedBotChecker);
	}
}
...
public Action:PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ((PlayerEntered) && (!MissionLostSet))
		LogMessage("PlayerSpawn Timer started")
	{
		//Start the Safe Room Timer for Coop/Survival, MI 5 Code, two timers for survival/coop and the timer for bots to pass cresendos, MI 5
		if (GameMode == 1)
		{
