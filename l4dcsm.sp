...
* 
* Version 3.0
* 		- Fixed additional crashes with Bill
* 		- Fixed bug where common infected would not spawn if someone was playing as Bill
* 		- Fixed bug where Bill would not become Nick when a round was over
* 
* Version 2.5a/2.5b
...
public _ConVarChange__GameMode(Handle:convar, const String:oldValue[], const String:newValue[]) 
{
	...
	#if DEBUG
	PrintToChatAll("GameMode Changed");
	LogMessage("GameMode Changed");
	#endif
}
...
public Action:Event_PlayerFirstSpawned(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	// If player has already entered the start area, don't execute the rest of this code
	...
	#if DEBUG
	PrintToChatAll("%N Player first spawned", client);
	LogMessage("%N Player first spawned", client);
	#endif
	...
	if (g_iGameMode == GAMEMODE_COOP && StrEqual(CurrentMap, "c6m2_bedlam", true) && (GET_ZOEY_NETPROP || GET_FRANCIS_NETPROP || GET_LOUIS_NETPROP))
	{
		...
		g_iSavedCharacter[client] = 0;
	}
	else
	g_iSavedCharacter[client] = GetEntProp(client, Prop_Send, "m_survivorCharacter");
	
	// restore the player's character. If the map was The Passing and there was a mission failure, restore the L4D1 survivors.
	
	if (!g_bWasBill[client] && !g_bWasL4D1Survivor[client])
	{
		#if DEBUG
		PrintToChatAll("%N Was a L4D2 character, Restoring", client);
		LogMessage("%N Was a L4D2 character, Restoring", client);
		#endif
	...
	{
		#if DEBUG
		PrintToChatAll("%N was L4D1 Survivor, changing them back", client);
		LogMessage("%N was L4D1 Survivor, changing them back", client);
		#endif
...
public Action:Event_GameEnded(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	LogMessage("The Safe Room door is closed and all survivors are in.");
	#endif
	
	// This event kills all of Bill's timers, and saves each player's character. It also changes all Bills to Nick in order to prevent a crash. Also prevents glitches on The Passing with the
	// L4D1 Survivors.
		...
		if (IsClientInGame(client))
		{
			if (GetClientTeam(client) == TEAM_SURVIVORS)
			{
				if (!IsFakeClient(client))
				{
					...
					if (GET_BILL_NETPROP || g_bWasBill[client])
					{
...
public Action:Event_MissionLost(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	#if DEBUG
	PrintToChatAll("The Mission has been lost.");
	LogMessage("The Mission has been lost.");
	#endif
...
public Action:Event_BillFinaleStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	#if DEBUG
	PrintToChatAll("Finale has started, removing the Bill fix.");
	LogMessage("Finale has started, removing the Bill fix.");
	#endif
	...
	for (new client=1;client<=MaxClients;client++)
	{
		if (IsClientInGame(client))
		{
			if (GetClientTeam(client) == TEAM_SURVIVORS)
			{
				if (GET_BILL_NETPROP || g_bWasBill[client] || g_iVersusOldSurvivor[client] == 4)
				{
					g_bWasBill[client] = false;
					new anyclient = GetAnyValidClient();
					CheatCommand(anyclient, "director_start");
					SET_CHARACTER_BILL
				}
			}
		}
...
{
	// Prevents a horrific glitch with the L4D1 Survivors on The Passing finale
	
	#if DEBUG
	PrintToChatAll("Finale Started.");
	LogMessage("Finale Started.");
	#endif
...
public Action:Event_SomeoneIsVoting(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	#if DEBUG
	PrintToChatAll("Someone has started a vote. Removing Bill from play.");
	LogMessage("Someone has started a vote. Removing Bill from play.");
	#endif
	...
	if (IsClientInGame(client)) 
	{
		if (GetClientTeam(client) == TEAM_SURVIVORS && g_bAnnounce)
		{
			switch (g_iSurvivorModels) 
			{
				...
				PrintToChat(client, "\x03[CSM]\x01 Type \x05!csm\x01 in chat to change your model (outside apperance).");
				case CSM_SURVIVOR_CHARACTER_ONLY:
				PrintToChat(client, "\x03[CSM]\x01 Type \x05!csm\x01 in chat to change your character (outside appearance, sounds and portrait).");
				case CSM_SURVIVOR_MODEL_AND_CHARACTER:
				PrintToChat(client, "\x03[CSM]\x01 Type \x05!csm\x01 in chat to change your character (outside appearance, sounds and portrait) or model (outside apperance).");
			}
			
			switch (g_iInfectedModels) 
			{
				case CSM_INFECTED_MODEL_ONLY:
				PrintToChat(client, "\x03[CSM]\x01 Type \x05!csm\x01 in chat to change your model to an infected (outside apperance).");
				case CSM_INFECTED_CHARACTER_ONLY:
				PrintToChat(client, "\x03[CSM]\x01 Type \x05!csm\x01 in chat to change your character to an infected (outside appearance, abilities and sounds).");
				case CSM_INFECTED_CHARACTER_AND_MODEL:
				PrintToChat(client, "\x03[CSM]\x01 Type \x05!csm\x01 in chat to change your character (outside appearance, abilities and sounds) or model (outside apperance) to an infected.");
			}
		}
	}
}
...
public Action:InitiateMenuAdmin(client, args) 
{
	...
	{
		ReplyToCommand(client, "[CSM] Character Select Menu has no options available.");
		return;
	}
...
stock SetCharacter(client, character) 
{
	...
	// Make sure that the Bill variable is reset
...
{
	#if DEBUG
	PrintToChatAll("%N has selected Bill. Starting his Bill timer.", client);
	LogMessage("%N has selected Bill. Starting his Bill timer.", client);
	#endif
	...
	g_bWasBill[client] = false;
	new anyclient = GetAnyValidClient();
	CheatCommand(anyclient, "director_stop");
	SET_CHARACTER_BILL
}
...
public Action:Timer_SpawnZombies(Handle:Timer)
{
	...
	#if DEBUG
	PrintToChatAll("Bill timer stopped and zombies restored.");
	LogMessage("Bill Timer stopped and zombies restored.");
	#endif
}
...
public Action:Timer_RestoreBill(Handle:Timer)
{
	...
	for (new client=1;client<=MaxClients;client++)
	{
		...
		if (GetClientTeam(client) == TEAM_SURVIVORS)
		{
			if (g_bWasBill[client] || g_iVersusOldSurvivor[client] == 4)
			{
				billexists = true;
				g_bWasBill[client] = false;
				new anyclient = GetAnyValidClient();
	...
	#if DEBUG
	PrintToChatAll("Bill timer restored and zombies stopped.");
	LogMessage("Bill Timer restored and zombies stopped.");
	#endif
}
...
stock SetUpInfected(param1, Infected)
{
	...
	if (g_bAnnounce)
	{
		...
		{
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[CSM]\x01 Your character is now \x04%s\x01! You cannot change again until you respawn.", g_sCharacters[Infected]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[CSM]\x01 Your character is now \x04%s\x01! You only have \x051\x01 change left.", g_sCharacters[Infected]);
			else
			PrintToChat(param1, "\x03[CSM]\x01 Your character is now \x04%s\x01! You have \x05%i\x01 changes left.", g_sCharacters[Infected], g_iClientChangeLimit[param1]);
		} 
		else 
		{
			PrintToChat(param1, "\x03[CSM]\x01 Your character is now \x04%s\x01!", g_sCharacters[Infected]);
		}
	}
...
stock SetUpInfectedModel(param1, Infected)
{
	...
	if (g_bAnnounce)
	{
		...
		{
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[CSM]\x01 Your character is now \x04%s\x01! You cannot change again until you respawn.", g_sCharacters[Infected+8]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[CSM]\x01 Your character is now \x04%s\x01! You only have \x051\x01 change left.", g_sCharacters[Infected+8]);
			else
			PrintToChat(param1, "\x03[CSM]\x01 Your model is now \x04%s\x01! You have \x05%i\x01 changes left.", g_sCharacters[Infected+8], g_iClientChangeLimit[param1]);
		}
		else
		{
			PrintToChat(param1, "\x03[CSM]\x01 Your model is now \x04%s\x01!", g_sCharacters[Infected+8]);
		}
	}
}
...
stock SetUpCommonModel(param1, Infected)
{
	...
	if (g_bAnnounce)
	{
		if (g_iClientChangeLimit[param1] <= CSM_CHANGE_MESSAGE)
		{
			...
			PrintToChat(param1, "\x03[CSM]\x01 Your model is now \x04%s\x01! You have \x05%i\x01 changes left.", g_sCharacters[Infected], g_iClientChangeLimit[param1]);
		}
		else
		{
			PrintToChat(param1, "\x03[CSM]\x01 Your model is now \x04%s\x01!", g_sCharacters[Infected]);
		}
	}
}
